...
kernel void initialize(
            local TYPE* restrict A __attribute__((numbanks(NUMBANKS),bankwidth(BANKWIDTH),local_mem_size(SIZE*SIZEOF))),
            const ulong elements)
{
  for (ulong i = 0; i < elements; ++i)
  {
    #pragma unroll UNROLL
    for (ulong j = 0; j < UNROLL; ++j)
    {
      A[i][j]=1;
    }
  }
...
kernel void ocl_kernel(
            const ulong trials,
            const ulong elements,
            local TYPE* restrict A[UNROLL] __attribute__((numbanks(NUMBANKS),bankwidth(BANKWIDTH),local_mem_size(SIZE*SIZEOF))),
